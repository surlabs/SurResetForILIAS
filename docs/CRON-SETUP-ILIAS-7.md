# Configuracion de Cron en ILIAS 7

Este documento describe como activar la ejecucion periodica de cronjobs en ILIAS 7, sin depender de componentes adicionales.
...
3. Crea una nueva tarea con estos valores:
   ...
   - `Arguments`: `USUARIO_ILIAS PASSWORD_ILIAS CLIENT_ID`
...
```bash
* * * * * /usr/bin/php -f '/path/RootPathILIAS/cron/cron.php' -- 'USUARIO_ILIAS' 'PASSWORD_ILIAS' 'CLIENT_ID' >> $HOME/ilias-cron.log 2>&1
```

## 4. Ejecucion manual inicial

```bash
sudo -u system_user /usr/bin/php '/path/RootPathILIAS/cron/cron.php' -- 'USUARIO_ILIAS' 'PASSWORD_ILIAS' 'CLIENT_ID'
```
